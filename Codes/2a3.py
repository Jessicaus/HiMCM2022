...
for i in range(len(yearspre)):
    yearsy1.append([yearspre[i]])
print(len(yearsy1))
yearsy=yearsy1[:46]


from sklearn.svm import SVR
regressor = SVR(kernel="poly").fit(yearsy,temperature)
prediction=regressor.predict(yearsy1)

dct=dict(zip(yearspre,prediction))
...
ax.scatter(yearslast,temperaturelast,label="Testing")
#ax.plot(years_test,prediction,color='black',label="Predicted")
ax.grid()
ax.set_xlabel("Year")
ax.set_ylabel("Land-Ocean Temperature (Degrees Celsius)")
...
regressor2 =SVR(kernel="rbf").fit(yearsy,temperature)
prediction2=regressor2.predict(yearsy1)

dct2=dict(zip(yearspre,prediction2))
...
regressor3 =SVR(kernel="linear").fit(yearsy,temperature)
prediction3=regressor3.predict(yearsy1)

dct3=dict(zip(yearspre,prediction3))
dct3=OrderedDict(sorted(dct3.items()))

fig, ax = plt.subplots(figsize=(7, 5))
ax.scatter(years,temperature)
ax.plot(dct3.keys(),dct3.values(),color="green")
ax.scatter(yearslast,temperaturelast,label="Testing")
#ax.plot(years_test,prediction,color='black',label="Predicted")
ax.grid()
ax.set_xlabel("Year")
...
resid,residsum=residualtest(prediction[46:],temperaturelast)
print(residsum)
...
resid,residsum=residualtest(prediction2[46:],temperaturelast)
print(residsum)

#SSM for SVR Linear Model
resid,residsum=residualtest(prediction3[46:],temperaturelast)
print(residsum)

# In[83]:

#SSM For Linear Model
resid,residsum=residualtest(linearmodel[46:],temperaturelast)
#print(len(resid))
#print(resid)
print(residsum)
...
def func2(x,y):
    return p[2] * y + p[1] * x + p[0]


# In[21]:


modelt=list(map(func2,years,np.square(years)))
modeltr=list(map(func2,yearslast,np.square(yearslast)))
print(modeltr)
...
resid,residsum=residualtest(modeltr,temperaturelast)
print(residsum)
...
fig, ax = plt.subplots(figsize=(7, 5))
ax.scatter(years,temperature)
ax.plot(years,modelt,color="green")
ax.plot(yearslast,modeltr,color="red")
ax.scatter(yearslast,temperaturelast,label="Testing")
#ax.plot(years_test,prediction,color='black',label="Predicted")
ax.grid()
ax.set_xlabel("Year")
